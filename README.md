...
- **새 기능을 개발하지 않는다**
- 프로젝트를 분석하여 우선순위대로 **자율 반복** 수행
- High Priority 문제가 남아있는 동안 계속한다
...
**작업 루프:** 최우선 문제 선택 → 수정 → 테스트 → Commit & Push → `PROJECT_STATE` · `CHANGELOG` 업데이트 → 반복
...
- **기능을 추가하지 않는다**
- 실제 사용자 경험이 좋아지는 경우에만 개선 적용
- **"계속할까요?" 질문 금지** — 개선 가능 항목이 없을 때까지 반복

**개선 항목:** 사용자 흐름, 버튼 위치, 정보 계층, 여백, 가독성, 반응형, 접근성, 로딩·빈·에러 화면, 피드백 메시지, 사용성

**작업 루프:** 개선 선택 → 수정 → 테스트 → Commit & Push → 문서 업데이트 → 반복
...
- Cursor는 ChatGPT(CTO) 관리 아래 **자율적으로** 개발·QA·UI/UX를 수행한다
- ChatGPT Task가 없어도 스스로 프로젝트를 개선한다
- **"다음 작업을 진행할까요?" / "계속할까요?" 질문 금지**
- 종료 조건이 모두 충족될 때까지 작업을 계속한다
...
| v1.0.4 | UI/UX Sprint, 자율 개발 사이클 통합 (현재) |
